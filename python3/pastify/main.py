...
class Pastify(object):
    ...
    def get_file_name(self):
        return vim.exec_lua(
            'return require("pastify").getFileName()'
        )
    ...
    def paste_text(self, after) -> None:
        ...
        if options['save'] in ["local", "local_file"]:
            ...
            filename = path.splitext(file_name)[0]
        ...
        if self.config['opts']['save'] in ["local", "local_file"]:
            assets_path = path.abspath(path.join(local_path, self.get_image_path_name()))
            if not self.config['opts']['absolute_path']:
                ...
                current_file_path = path.dirname(vim.exec_lua('return vim.fn.expand("%:p")'))
